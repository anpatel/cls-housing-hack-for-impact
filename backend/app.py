...
def query_specific_files_for_other_info(files):
    ...
    legal_basis = initialize_document_index('argument_result_search', documents_to_query).query('pretend i am an attorney starting to research a new case. based on the context you have in your knowledge base, please provide a legal basis citing relevant housing ordinances from East Palo Alto and Mountain View. please ensure citations are no longer than 100 words each, and include the citation sources case title and year.').response
    issue_type = initialize_document_index('argument_result_search', documents_to_query).query('based on the context you have in your knowledge base, what is the issue type (such as "mold" or "water leak") of this case? please just provide a phrase.').response
    reasonable_outcomes = initialize_document_index('argument_result_search', documents_to_query).query('based on relevant past cases, what are the reasonable outcomes of this case? select 1-3 options between: "File a Petition," "Issue Demand Letter," or "Seek Mediation")').response
